	...
	public class CompoundAssign : Assign
	{
		...
		protected override Expression DoResolve (ResolveContext ec)
		{
			...
			if (target is DynamicMemberAssignable) {
				...
				var binder_flags = ALBinderFlags.ValueFromCompoundAssignment;
				...
				if (ec.HasSet (ResolveContext.Options.CheckedScope))
					binder_flags |= ALBinderFlags.CheckedContext;
		...
		protected override Expression ResolveConversions (ResolveContext ec)
		{
			...
			if (source.Type.BuiltinType == BuiltinTypeSpec.Type.Dynamic) {
				...
				return new SimpleAssign (target, new DynamicConversion (target_type, ALBinderFlags.ConvertExplicit, arg, loc), loc).Resolve (ec);
			}
