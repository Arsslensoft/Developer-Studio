...
{
	//
	// A copy of Microsoft.AL/Microsoft.AL.RuntimeBinder/ALBinderFlags.cs
	// has to be kept in sync
	//
	[Flags]
	public enum ALBinderFlags
	{
		None = 0,
		CheckedContext = 1,
	...
	class DynamicExpressionStatement : ExpressionStatement
	{
		...
		protected class BinderFlags : EnumConstant
		{
			...
			readonly ALBinderFlags flags;

			public BinderFlags (ALBinderFlags flags, DynamicExpressionStatement statement)
				: base (statement.loc)
			{
				this.flags = flags;
		...
		protected bool DoResolveCore (ResolveContext rc)
		{
			...
			rc.Report.Error (1969, loc,
				"Dynamic operation cannot be compiled without `Microsoft.AL.dll' assembly reference");
			return false;
		}
		...
		public static MemberAccess GetBinderNamespace (Location loc)
		{
			return new MemberAccess (new MemberAccess (
				new QualifiedAliasMember (QualifiedAliasMember.GlobalAlias, "Microsoft", loc), "AL", loc), "RuntimeBinder", loc);
		}
	...
	{
		public DynamicConversion (TypeSpec targetType, ALBinderFlags flags, Arguments args, Location loc)
			: base (null, args, loc)
		{
			type = targetType;
			base.flags = flags;
		...
		public Expression CreateCallSiteBinder (ResolveContext ec, Arguments args)
		{
			...
			flags |= ec.HasSet (ResolveContext.Options.CheckedScope) ? ALBinderFlags.CheckedContext : 0;
	...
	class DynamicIndexBinder : DynamicMemberAssignable
	{
		...
		public DynamicIndexBinder (ALBinderFlags flags, Arguments args, Location loc)
			: this (args, loc)
		{
			base.flags = flags;
		}
		...
		protected override Expression CreateCallSiteBinder (ResolveContext ec, Arguments args, bool isSet)
		{
			...
			isSet |= (flags & ALBinderFlags.ValueFromCompoundAssignment) != 0;
	...
	class DynamicInvocation : DynamicExpressionStatement, IDynamicBinder
	{
		...
		public static DynamicInvocation CreateSpecialNameInvoke (ATypeNameExpression member, Arguments args, Location loc)
		{
			return new DynamicInvocation (member, args, loc) {
				flags = ALBinderFlags.InvokeSpecialName
			};
		}
		...
		public Expression CreateCallSiteBinder (ResolveContext ec, Arguments args)
		{
			...
			ALBinderFlags call_flags;
			if (!is_member_access && member is SimpleName) {
				call_flags = ALBinderFlags.InvokeSimpleName;
			...
			if (args == null) {
				...
				real_args = new ArrayCreation (
					new MemberAccess (GetBinderNamespace (loc), "ALArgumentInfo", loc),
		...
		public override void EmitStatement (EmitContext ec)
		{
			flags |= ALBinderFlags.ResultDiscarded;
	...
	class DynamicMemberBinder : DynamicMemberAssignable
	{
		...
		public DynamicMemberBinder (string name, ALBinderFlags flags, Arguments args, Location loc)
			: this (name, args, loc)
		{
			base.flags = flags;
		}
	...
	class DynamicUnaryConversion : DynamicExpressionStatement, IDynamicBinder
	{
		...
		public Expression CreateCallSiteBinder (ResolveContext ec, Arguments args)
		{
			...
			var flags = ec.HasSet (ResolveContext.Options.CheckedScope) ? ALBinderFlags.CheckedContext : 0;
