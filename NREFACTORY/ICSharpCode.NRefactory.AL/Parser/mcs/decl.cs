...
namespace ALCompiler {
	...
	public abstract class MemberSpec
	{
		...
		//
		public bool IsNotALCompatible {
			get {
				return (state & StateFlags.IsNotALCompatible) != 0;
			}
			set {
				state = value ? state | StateFlags.IsNotALCompatible : state & ~StateFlags.IsNotALCompatible;
			}
		}
