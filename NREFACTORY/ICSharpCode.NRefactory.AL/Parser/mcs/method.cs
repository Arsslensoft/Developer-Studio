	...
	public sealed class MethodSpec : MemberSpec, IParametersMember
	{
		...
		public override string GetSignatureForError ()
		{
			...
			if (IsConstructor) {
				...
				if (targs != null)
					name += "<" + TypeManager.ALName (targs) + ">";
				else if (IsGeneric)
					name += "<" + TypeManager.ALName (GenericDefinition.TypeParameters) + ">";
			}
...
//				return ginstance;
	...
	public abstract class MethodOrOperator : MethodCore, IMethodData, IMethodDefinition
	{
		...
		static readonly string[] attribute_targets = new string [] { "method", "backwith" };
			...
			if (a.Type == pa.Conditional) {
				...
				if (MethodData.implementing != null) {
					...
					Report.Error (629, Location, "Conditional member `{0}' cannot implement interface member `{1}'",
						GetSignatureForError (), TypeManager.ALSignature (MethodData.implementing));
					return;
				}
		...
		protected override Expression DoResolve (ResolveContext ec)
		{
			...
			using (ec.Set (ResolveContext.Options.BaseInitializer)) {
				...
				if (this is ConstructorBaseInitializer) {
					...
					//
					// It is legal to have "self" initializers that take no arguments
					// in structs
					//
						...
						if (!method.IsAccessor) {
							if (implementing.IsAccessor) {
								...
								container.Compiler.Report.Error (470, method.Location,
									...
									method.GetSignatureForError (), TypeManager.ALSignature (implementing));
							}
							...
							if (!implementing.IsAccessor) {
								...
								container.Compiler.Report.Error (686, method.Location,
									...
									method.GetSignatureForError (), TypeManager.ALSignature (implementing), container.GetSignatureForError ());
							} else {
