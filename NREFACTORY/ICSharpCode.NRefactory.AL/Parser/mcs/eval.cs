...
namespace ALCompiler
{
	...
	public class Evaluator {
		...
		///   this is the ALCompiler.InteractiveBase class
		...
		public string Compile (string input, out CompiledMethod compiled)
		{
			...
			lock (evaluator_lock){
				...
				bool partial_input;
				ALParser parser = ParseString (ParseMode.Silent, input, out partial_input);
		...
		public string [] GetCompletions (string input, out string prefix)
		{
			...
			lock (evaluator_lock){
				...
				bool partial_input;
				ALParser parser = ParseString (ParseMode.GetCompletions, input, out partial_input);
				if (parser == null){
					return null;
				}
		...
		//
		// Parses the string @input and returns a ALParser if succeeful.
		//
		...
		//
		ALParser ParseString (ParseMode mode, string input, out bool partial_input)
		{
			partial_input = false;
			Reset ();
			...
			ALParser parser = new ALParser (seekable, source_file, new ParserSession ());
	...
	public class InteractiveBase {
		...
		/// </summary>
		public static string Prompt             = "AL> ";
